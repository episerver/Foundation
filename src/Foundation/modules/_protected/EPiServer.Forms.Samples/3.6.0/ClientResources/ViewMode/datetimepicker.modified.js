...
(function ($) {
    /*!
     * jQuery UI DateTimePicker 1.0.0     
     *
    ...
    $.extend(DateTimePicker.prototype, {
        ...
        _doKeyDown: function (event) {
            var onSelect, dateStr, sel,
                inst = $.datetimepicker._getInst(event.target),
                handled = true,
            ...
            if ($.datetimepicker._datepickerShowing) {
                switch (event.keyCode) {
                    ...
                    case 13: sel = $("td." + $.datetimepicker._dayOverClass + ":not(." +
                                        $.datetimepicker._currentClass + ")", inst.dpDiv);
                        if (sel[0]) {
                            $.datetimepicker._selectDay(event.target, inst.selectedMonth, inst.selectedYear, sel[0]);
                        }

                        onSelect = $.datetimepicker._get(inst, "onSelect");
                        if (onSelect) {
                            dateStr = $.datetimepicker._formatDate(inst);
                    ...
                    case 37: if (event.ctrlKey || event.metaKey) {
                        ...
                        if (event.originalEvent.altKey) {
                            $.datetimepicker._adjustDate(event.target, (event.ctrlKey ?
								-$.datetimepicker._get(inst, "stepBigMonths") :
								-$.datetimepicker._get(inst, "stepMonths")), "M");
                        }
        ...
        _doKeyPress: function (event) {
            ...
            if ($.datetimepicker._get(inst, "constrainInput")) {
                chars = $.datetimepicker._possibleChars($.datetimepicker._get(inst, "dateFormat"));
        ...
        _doKeyUp: function (event) {
            ...
            if (inst.input.val() !== inst.lastVal) {
                try {
                    $.datetimepicker._setTimeFromField(inst);
                    date = $.datetimepicker.parseDate($.datetimepicker._get(inst, "dateFormat"),
                        (inst.input ? inst.input.val() : null),
                        $.datetimepicker._getFormatConfig(inst));
        ...
        _showDatepicker: function (input) {
            ...
            inst = $.datetimepicker._getInst(input);
            if ($.datetimepicker._curInst && $.datetimepicker._curInst !== inst) {
                ...
                if (inst && $.datetimepicker._datepickerShowing) {
                    $.datetimepicker._hideDatepicker($.datetimepicker._curInst.input[0]);
                }
            }

            beforeShow = $.datetimepicker._get(inst, "beforeShow");
            ...
            offset = { left: $.datetimepicker._pos[0], top: $.datetimepicker._pos[1] };
            ...
            inst.dpDiv.css({
                position: ($.datetimepicker._inDialog && $.blockUI ?
                    "static" : (isFixed ? "fixed" : "absolute")), display: "none",
                left: offset.left + "px", top: offset.top + "px"
            });

            if (!inst.inline) {
                showAnim = $.datetimepicker._get(inst, "showAnim");
                duration = $.datetimepicker._get(inst, "duration");
                inst.dpDiv.css("z-index", datepicker_getZindex($(input)) + 1);
                $.datetimepicker._datepickerShowing = true;

                if ($.effects && $.effects.effect[showAnim]) {
                    inst.dpDiv.show(showAnim, $.datetimepicker._get(inst, "showOptions"), duration);
                } else {
                    inst.dpDiv[showAnim || "show"](showAnim ? duration : null);
                }

                if ($.datetimepicker._shouldFocusInput(inst)) {
                    inst.input.focus();
                }
        ...
        _updateDatepicker: function (inst) {
            ...
            if (inst === $.datetimepicker._curInst && $.datetimepicker._datepickerShowing && $.datetimepicker._shouldFocusInput(inst)) {
                inst.input.focus();
            }
        ...
        _checkExternalClick: function (event) {
            ...
            var $target = $(event.target),
                inst = $.datetimepicker._getInst($target[0]);
            ...
            if ((($target[0].id !== $.datetimepicker._mainDivId &&
                    ...
                    !$target.closest("." + $.datetimepicker._triggerClass).length &&
                    $.datetimepicker._datepickerShowing && !($.datetimepicker._inDialog && $.blockUI))) ||
                ($target.hasClass($.datetimepicker.markerClassName) && $.datetimepicker._curInst !== inst)) {
                $.datetimepicker._hideDatepicker();
            }
        },
        ...
        _selectDay: function (id, month, year, td) {
            ...
            var timeStr = this._selectTime(id);
            
            this._selectDate(id, this._formatDate(inst,
                inst.currentDay, inst.currentMonth, inst.currentYear), timeStr);
        },
        ...
        _selectTime: function (id) {
            ...
            inst.currentHour = $(".ui-datepicker-hour", inst.dpDiv).val();
            inst.currentMinute = $(".ui-datepicker-minute", inst.dpDiv).val();
            inst.currentPeriod = $(".ui-datepicker-period", inst.dpDiv).val();

            var timeStr = inst.currentHour + ":" + inst.currentMinute + " " + inst.currentPeriod;
            return timeStr;
        },
        ...
        /* Update date time value text box when user select date, time. */
        _updateInputValue: function (id, dateStr, timeStr) {
            var target = $(id),
                inst = this._getInst(target[0]),
                dateTimeStr = inst.settings.type == 'datepicker' ? dateStr
                    : inst.settings.type == 'timepicker' ? timeStr : dateStr + " " + timeStr;
            if (inst.input) {
                inst.input.val(dateTimeStr);
            }
        },

        /* Update the input field with the selected date. */
        _selectDate: function (id, dateStr, timeStr) {
            var onSelect,
                target = $(id),
                inst = this._getInst(target[0]);

            this._updateInputValue(id, dateStr, timeStr);
        ...
        _setTimeFromField: function (inst) {
            ...
            if (inst.settings.type == 'datepicker') {
                return;
            }

            // default value for hour, minute, period
            inst.currentHour = 12;
            inst.currentMinute = "00";
            inst.currentPeriod = "AM";

            var valueStr = inst.lastVal = inst.input ? inst.input.val() : null;
            if (!valueStr) {
                return;
            }

            var arr = valueStr.split(' ');
            var hourMinute = inst.settings.type == 'timepicker' ? arr[0] : arr[1];
            var period = arr[arr.length - 1];

            if (!hourMinute || !period) {
                return;
            }

            var hourMinuteArr = hourMinute.split(':');
            inst.currentHour = hourMinuteArr[0];
            inst.currentMinute = hourMinuteArr[1];
            inst.currentPeriod = period;
        },
        ...
        _attachHandlers: function (inst) {
            ...
            inst.dpDiv.find("[data-handler]").map(function () {
                var handler = {
                    prev: function () {
                        $.datetimepicker._adjustDate(id, -stepMonths, "M");
                    },
                    next: function () {
                        $.datetimepicker._adjustDate(id, +stepMonths, "M");
                    },
                    ...
                    today: function () {
                        $.datetimepicker._gotoToday(id);
                    },
                    selectDay: function () {
                        $.datetimepicker._selectDay(id, +this.getAttribute("data-month"), +this.getAttribute("data-year"), this);
                        return false;
                    },
                    selectMonth: function () {
                        $.datetimepicker._selectMonthYear(id, this, "M");
                        return false;
                    },
                    selectYear: function () {
                        $.datetimepicker._selectMonthYear(id, this, "Y");
                        return false;
                    },
                    selectHour: function () {
                        $.datetimepicker._selectHourMinutePeriod(id);
                    },
        ...
        /* Select hour, minute or period handler */
        _selectHourMinutePeriod: function (id) {
            var target = $(id),
                inst = this._getInst(target[0]),
                dateString = this._formatDate(inst, inst.currentDay, inst.currentMonth, inst.currentYear),
                timeStr = this._selectTime(id);
            this._updateInputValue(id, dateString, timeStr);
        },
        ...
        _generateHTML: function (inst) {
            ...
            switch (inst.settings.type) {
                case 'datetimepicker':
                    html += this._getTimePickerHtml(inst);
                    break;
                case 'datepicker':
                    break;
                case 'timepicker':
        ...
        _getTimePickerHtml: function (inst) {
        ...
        /* decide the current option is selected or not */
        _getSelectedAttribute: function (selectedValue, compareValue) {
            return selectedValue == compareValue ? "selected" : "";
        },

        _formatNumber: function (n, width, z) {
            z = z || '0';
            n = n + '';
    ...
    function datepicker_handleMouseover() {
        if (!$.datetimepicker._isDisabledDatepicker(datepicker_instActive.inline ? datepicker_instActive.dpDiv.parent()[0] : datepicker_instActive.input[0])) {
    ...
    $.fn.datetimepicker = function (options) {
        ...
        if ($("#" + $.datetimepicker._mainDivId).length === 0) {
            $("body").append($.datetimepicker.dpDiv);
        }
